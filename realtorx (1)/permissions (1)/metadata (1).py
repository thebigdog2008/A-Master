...
from realtorx.permissions2.metadata import BaseMetadata, PermissionsBasedMetadataMixin
...
class SimplifiedFSMTransitionActionMetadataMixin(FSMTransitionActionMetadataMixin):
    def _is_action_allowed(self, instance, request, view, action):
        ...
        allow_action = transition_permissions and all(
            permission.has_permission(request, view)
            and permission.has_object_permission(request, view, instance)
    ...
    def determine_actions(self, request, view):
        ...
        metadata["allowed_FSM_transitions"] = [
            action
            for action in metadata["allowed_FSM_transitions"]
            if self._is_action_allowed(instance, request, view, action)
        ]
        return metadata


class SimplePermissionBasedMetadata(
    SimplifiedFSMTransitionActionMetadataMixin,
