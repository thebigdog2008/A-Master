...
from realtorx.permissions.permissions import PermissionQ
from realtorx.utils.views import FSMTransitionActionMixin
...
class BaseSimplePermittedViewSetMixin(object):
    ...
    def check_write_permissions(self, instance=None, raise_error=True):
        ...
        read_only = not all(
            permission.has_permission(self.request, self)
    ...
    def get_serializer(self, instance=None, *args, **kwargs):
        ...
        kwargs["read_only"] = not self.check_write_permissions(
            not many and instance, raise_error=False
        )
    ...
    def check_write_permissions(self, instance=None, raise_error=True):
        ...
        if getattr(self, "parent", None) is None and not write_permissions:  # root view
            raise ImproperlyConfigured(
                "write_permissions should be defined for root view."
            )

        return super().check_write_permissions(
            instance=instance, raise_error=raise_error
        )
...
class SimplePermittedChildViewSetMixin(NestedViewSetMixin):
    ...
    def check_parent_write_permissions(self, raise_error=True):
        ...
        if parent_view is None or not isinstance(
            parent_view, BaseSimplePermittedViewSetMixin
        ):
            return True

        return parent_view.check_write_permissions(
            instance=self.get_parent_object(), raise_error=raise_error
        )

    def check_write_permissions(self, instance=None, raise_error=True):
        parent_write_allowed = self.check_parent_write_permissions(
...
class SimplePermittedViewSetMixin(
    SimplePermittedParentViewSetMixin,
    SimplePermittedChildViewSetMixin,
...
class SimplePermittedFSMTransitionActionMixin(FSMTransitionActionMixin):
    ...
    def get_transition_permissions(self, transition):
        return [
            (
                permission()
                if not isinstance(permission, (list, set))
                else PermissionQ(*permission)
            )
    ...
    def pre_transition(self, instance, action):
        ...
        allow_action = all(
            permission.has_permission(self.request, self)
            and permission.has_object_permission(self.request, self, instance)
